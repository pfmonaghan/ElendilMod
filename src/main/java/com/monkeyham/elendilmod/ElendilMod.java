...
@Mod(ElendilMod.MODID)
public class ElendilMod {
    // Define mod id in a common place for everything to reference
    public static final String MODID = "elendilmod";
    // Directly reference a slf4j logger
    public static final Logger LOGGER = LogUtils.getLogger();
    // Create a Deferred Register to hold Blocks which will all be registered under the "elendilmod" namespace

    // The constructor for the mod class is the first code that is run when your mod is loaded.
    // FML will recognize some parameter types like IEventBus or ModContainer and pass them in automatically.
    public ElendilMod(IEventBus modEventBus, ModContainer modContainer) {
        // Register the commonSetup method for modloading
        modEventBus.addListener(this::commonSetup);

        // Register ourselves for server and other game events we are interested in.
        // Note that this is necessary if and only if we want *this* class (ElendilMod) to respond directly to events.
        ...
        ModdedFeatures.register(modEventBus);
    }
    ...
    private void addCreative(BuildCreativeModeTabContentsEvent event) {
        if (event.getTabKey() == CreativeModeTabs.BUILDING_BLOCKS) {
            event.accept(ModBlocks.OSGILIATH_RUBBLE_1);
            event.accept(ModBlocks.OSGILIATH_ARCH_1);
            event.accept(ModBlocks.OSGILIATH_ARCH_PANE);
            event.accept(ModBlocks.OSGILIATH_ARCH_PANE_BOTTOM);
            event.accept(ModBlocks.OSGILIATH_RUBBLE_SLAB);
            event.accept(ModBlocks.OSGILIATH_RUBBLE_STAIRS);
            event.accept(ModBlocks.MORDOR_MERLON);
            event.accept(ModBlocks.OSGILIATH_ARCH_2);
